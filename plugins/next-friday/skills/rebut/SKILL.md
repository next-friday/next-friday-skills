...
| "Just apply the suggested diff"                      | It may break a repo gate the bot can't see, and the diff itself can be malformed — a real suggestion once injected an unrelated path into the prose. Verify against the current file and apply the point, not the literal diff. |
