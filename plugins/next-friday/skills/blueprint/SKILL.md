...
Creating more than one GitHub artifact in a session — including an epic plus its sub-issues — is a fan-out that a single design approval never authorizes. State the exact named set ("create these 5: epic X plus sub-issues a, b, c, d?") and get one explicit per-set yes before any `gh issue create`; approving the design is not authorizing the writes.
...
1. **Explore project context and check for collisions:** read files, docs, recent commits, AND open issues, branches (`gh issue list`, `git branch -a`, `gh issue develop <n> --list`), and open PRs for in-flight or overlapping work by other agents; surface any conflict to the user before designing or creating anything, and touch no artifact this session did not create
...
- **Visuals:** when a question is easier shown than told, such as mockups, layouts, or diagrams, offer visuals once for consent, then decide per question with one test: _would the user understand this better by seeing it than reading it?_ "Which wizard layout?" is yes; "what should this value default to?" is no. Present them as ASCII/markdown inline, Mermaid in the issue body (GitHub renders it), or HTML/SVG in a temp file the user can open
...
**Depth, not ceremony.** Include each element **only when it has content**. Never emit an empty heading as a ritual such as `Global Constraints: none` or `Out of scope: nothing`; that is the placeholder bloat the Self-Review below bans. Mandate the _check_, not a fixed set of headings.

Mandatory for Standard/Large, cheap and always carries information:

- **Decision log:** each material decision: choice + what it rules out + why; add an `Origin` note of user answer, constraint, or review finding only where the driver is non-obvious.
...
- **Global constraints:** when the repo has binding project-wide rules such as version floors, naming, or platform that every task inherits; copy them verbatim from the repo's own source of truth such as `CONTRIBUTING` or the template, never paraphrased.
...
1. **Native edit history:** GitHub keeps the full edit history of an issue body, viewable in the web UI under "edited ▾". This is the durable, team-visible backup and needs no extra work; it is also why the design lives in the body, never in a machine-local file a teammate cannot open.
2. **In-body version banner:** once the body has been revised after its first recording, add a banner at the very top of the body, above the first template heading (e.g. `### Goal`), not merely above the plan: `> v2: <what changed and why>`. Follow it with a short **Revisions** list mapping each change to its trigger such as a review finding or a user correction. A v1 body carries no banner.
...
- **The GitHub issue body is the single source of truth** (see the sections below). The issue is where work is discussed, reviewed, and tracked, and the body holds both the design and the implementation plan.
...
- **No relevant issue exists** → create one. Confirm before creating ("No existing issue found, create a new one titled `<X>`?"). One yes authorizes that one issue, not a batch; and an earlier constraint such as local-only or draft-only outranks a later broad "do it all", so reconfirm before widening the scope of outward writes.
...
- **Template exists** → you MUST use it: read the template file yourself and fill its sections with the agreed design, picking the right one if there are several and mapping the design onto its sections. If it contains a checklist, fill in what's known and leave the rest for the execution phase. Mechanics only: do NOT pass `--template` to `gh issue create`, because it is mutually exclusive with `--body`/`--body-file` and drops into an interactive editor an agent cannot drive. Deliver the filled template via `--body-file` instead; the issue content must follow the template's structure exactly.
- **YAML issue forms** (`*.yml` with a `body:` field list, the modern format) cannot be driven via the CLI at all, so parse the form schema yourself and emulate it: each field's `label` becomes a `### <Label>` heading in the body, in form order, required fields always filled. This matches how GitHub renders submitted forms. Respect the form's `title:` pattern and apply its `labels:` with `--label`.
...
Always write the body to a temp file and pass `--body-file`, because design bodies contain backticks, quotes, and `$` that break inline `--body` shell quoting.
...
- **Label by scope:** apply the labels that match the work's area/size. Only pass `--label` for labels that already exist (check `gh label list`); to add a new label, ask the user first, and never create labels unprompted in a shared repo.
- **Invite the relevant people:** assign/mention those who should read what's planned (`--assignee`, or `@mention` in the body). Determine who from CODEOWNERS or by asking the user, never by guessing.
...
Fix issues by **updating the issue body** (`gh issue edit <n> --body-file`) so the body always holds the complete current design; GitHub preserves the edit history. For multi-component designs or any newly created Standard or Large issue, also dispatch the reviewer subagent in `references/spec-issue-reviewer-prompt.md` for a deeper pass; skip it for a Trivial-tier change and for small single-component additions to an existing issue.
...
**Ordered tasks,** each a checkbox carrying:
...
Then, for a new or multi-task plan, dispatch the reviewer subagent in `references/plan-issue-reviewer-prompt.md` for a Buildability pass; fix what it finds by updating the issue body.
