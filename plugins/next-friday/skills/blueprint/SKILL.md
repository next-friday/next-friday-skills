...
**A Trivial-tier plan is one line.** For a Trivial change, the plan is its Done criterion plus the Verification command that proves it, with no Goal/Architecture/Tech-Stack header, no file map, and no task list. That single line IS the plan; the implement skill accepts it and does not treat it as missing.
