...
**If the push is rejected as non-fast-forward** because the remote branch gained commits you do not have — a teammate's push, a release bot's version bump, or a PR "Update branch" merge — this is recovery, not a block. Run `git fetch`, then `git rebase origin/<branch>` (or `git pull --rebase`) to replay your commits on the updated tip, resolve any conflict, re-run the gates, and push again. Never `--force`: it discards the remote's new commits. Tell this apart from the policy block below by the message — a non-fast-forward rejection says the remote is ahead; a guard hook or branch protection names the rule.
