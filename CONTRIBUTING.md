...
Then sign in to GitHub from the terminal, where a browser window guides you:
...
Every change starts here. Go to the repo on GitHub, click **New issue**, pick one of the Bug, Feature, Design, or Task templates, and fill it in.
...
- `type` is one of `build`, `chore`, `ci`, `docs`, `feat`, `fix`, `perf`, `refactor`, `revert`, `setup`, `style`, or `test`; `.commitlintrc.json` is the source of truth
- `scope` is the area you're touching, such as `blueprint`, `implement`, `ci`, or `repo`
...
- the subject, meaning the description after the colon, is **50 characters or fewer**

If your message breaks a rule, the commit is **rejected** with an explanation. Fix the message and commit again. Never use `git add -A` / `git add .`, which sweeps in unrelated files, and never `--no-verify`, which skips the hook the gate exists to enforce.

### Step 5: Add a changeset, for behavior changes only

If your change affects how the plugin behaves, meaning a `feat`, `fix`, `perf`, or `refactor`, record a changeset so it shows up in the next release:

```sh
pnpm changeset
```

It asks two things: the **bump type**, one of patch, minor, or major as described under [versioning](#versioning) below, and a one-line **summary**. It then writes a small markdown file under `.changeset/`. Commit that file with your change.
...
- **All green** → the PR still needs **one approving review from a code owner** before it can merge. The repo requires it, and no one merges their own unreviewed work.

Merge method is **squash only**. After merge, the branch is deleted automatically and the issue closes.

## Releasing

You never bump the version by hand. Whenever changesets land on `main`, the `release.yml` workflow, which runs `changesets/action`, automatically opens or updates a **"version packages" PR**. A pending release is always visible and impossible to forget.
...
Versions follow [SemVer](https://semver.org), expressed as `MAJOR.MINOR.PATCH`:
...
While the plugin is `0.x`, it's still stabilizing, so even breaking changes bump the minor. The jump to `1.0.0` is a deliberate "this is stable" declaration, made when the skills have settled and seen real use. Changesets computes the next number for you; you only choose patch/minor/major.
...
- Allowed, since these are not prose comments: shebangs (`#!`), YAML/JSON keys, license headers, and machine-meaningful directives such as a lint-disable pragma.
...
For how these pieces fit together, see [Architecture](./docs/architecture.md). It covers the shared script library and its exit-code contracts, reference resolution, the SessionStart hook, and how the three skills compose.
